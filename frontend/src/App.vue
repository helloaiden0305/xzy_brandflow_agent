<template>
  ...
  <div v-else class="app-layout">
        ...
        <div class="thread-list">
          ...
          >
            <div class="thread-item-header">
              ...
              <span class="thread-title">{{ thread.topic_direction || '未命名任务' }}</span>
            </div>
    ...
    <div class="card">
      <div class="workflow-steps">
        <div class="step" :class="{ active: currentStep === 0, completed: currentStep > 0 }">
          ...
          <div class="step-title">输入方向</div>
        </div>
        <div class="step" :class="{ active: currentStep === 1, completed: currentStep > 1 }">
          <div class="step-icon">2</div>
          <div class="step-title">选择主题</div>
        </div>
        <div class="step" :class="{ active: currentStep === 2, completed: currentStep > 2 }">
          <div class="step-icon">3</div>
          <div class="step-title">审核初稿</div>
        </div>
        <div class="step" :class="{ active: currentStep === 3, completed: currentStep > 3 }">
          <div class="step-icon">4</div>
          <div class="step-title">生成素材</div>
        </div>
    ...
    <div v-if="currentStep === 0" class="card">
      <div class="card-title">输入宣传方向</div>
      <div style="display: flex; gap: 12px;">
        <input 
          v-model="topicDirection" 
          class="input" 
          placeholder="请输入宣传方向，例如：技术成果宣传、招聘宣传、活动复盘、内部公告"
    ...
    <div v-if="currentStep === 1" class="card">
      <div class="card-title">选择宣传主题</div>
      <!-- 流式生成中：显示实时内容 -->
      <div v-if="loading && streamingTopicsText" class="streaming-content">
        <div class="streaming-label">AI 正在生成宣传主题...</div>
      ...
      <div v-else-if="loading && generatedTopics.length === 0 && !streamingTopicsText" class="loading">
        ...
        <p style="margin-top: 12px;">AI 正在生成宣传主题...</p>
      </div>
      <!-- 已生成宣传主题列表 -->
      <div v-else class="topic-list">
        ...
        <div v-if="loading && generatedTopics.length > 0" class="loading-small" style="margin-top: 12px;">
          <div class="loading-spinner-small"></div>
          <span style="margin-left: 8px; color: #666;">正在生成更多宣传主题...</span>
        </div>
      </div>
      <div class="btn-group">
        <button 
    ...
    <div v-if="currentStep === 2" class="card">
      <div class="card-title">审核宣传初稿</div>
      <div v-if="loading && !articleContent" class="loading">
        <div class="loading-spinner"></div>
        <p style="margin-top: 12px;">AI 正在撰写宣传初稿...</p>
      </div>
      <template v-else>
        ...
        <div class="feedback-section">
          ...
          <textarea 
            v-model="feedback" 
            class="textarea" 
            placeholder="如果需要驳回，请输入修改意见，例如口径调整、补充事实、降低营销感..."
            :disabled="loading"
          ></textarea>
        </div>
        <div class="btn-group">
    ...
    <div v-if="currentStep === 3" class="card">
      <div class="card-title">正在生成传播素材</div>
      
      <!-- 显示视觉摘要 -->
      <div v-if="visualPoints.length > 0" class="visual-summary">
        <div class="visual-summary-title">视觉摘要（AI 提取的传播素材要点）</div>
      ...
      <div class="loading">
        <div class="loading-spinner"></div>
        <p style="margin-top: 12px;">AI 正在根据视觉摘要生成传播素材...</p>
      </div>
    </div>
    ...
    <div v-if="currentStep === 4">
      <div class="card">
        <div class="card-title">最终宣传内容</div>
    ...
    <div v-if="threadId" class="card" style="margin-top: 20px; background: #fafafa;">
      <div class="card-title" style="font-size: 14px; color: #666;">任务信息</div>
...
function getLogMessage(type, data) {
  const messages = {
    'init': `宣传任务初始化 - Thread ID: ${data?.thread_id || ''}`,
    'start': `开始执行 - 模式: ${data?.stream_mode || ''}`,
    'resume': `恢复宣传任务 - 操作: ${data?.action || ''}`,
...
const generatedTopics = ref([])  // 宣传主题列表
const selectedTopic = ref('')
const streamingTopicsText = ref('') // 流式生成宣传主题时的实时文本
...
function getNodeDisplayName(nodeName) {
  const nameMap = {
    'plan_topics': '宣传主题规划',
    'write_draft': '宣传初稿撰写',
    'extract_visuals': '提取视觉摘要',
    'generate_images': '生成传播素材'
  }
  return nameMap[nodeName] || nodeName
}

// 页面加载时获取历史任务（在 checkAuth 之后）
...
// 主题策划阶段使用非流式结构化输出，但通过 SSE 获取进度更新
async function handleStart() {
  ...
  try {
    ...
    // 主题策划阶段后端使用非流式结构化输出，SSE 主要用于进度反馈
    await streamStartWorkflow(topicDirection.value, {
      ...
      // LLM token 流式输出（events 模式）- 主题策划阶段是结构化输出，不会有 token
      onLlmToken: (content) => {
        // 主题策划阶段使用结构化输出，一般不会触发此回调
        // 但保留以兼容旧行为
      ...
      onDone: (data) => {
        ...
        loading.value = false
        showMessage('宣传主题已生成，请选择一个继续', 'success')
        fetchThreadList()
      },
      // 错误事件
      onError: (errorMsg) => {
    ...
    }, 'updates')  // 主题策划阶段使用 updates 模式（非流式结构化输出）
...
// 选择宣传主题 (使用 LangGraph 官方 graph.astream)
  ...
  try {
    ...
    await streamSelectTopic(threadId.value, selectedTopic.value, {
      ...
      onDone: (data) => {
        ...
        loading.value = false
        showMessage('宣传初稿已生成，请审核', 'success')
      },
      // 错误事件
      onError: (errorMsg) => {
        console.error('选择宣传主题失败:', errorMsg)
  ...
  try {
    await streamRejectArticle(threadId.value, currentFeedback, {
      ...
      onDone: (data) => {
        ...
        loading.value = false
        showMessage('宣传初稿已重写，请重新审核', 'info')
      },
      // 错误
      onError: (errorMsg) => {
...
async function handleSwitchThread(targetThreadId) {
  ...
  try {
    ...
    showMessage('已切换到历史宣传任务', 'success')
  } catch (error) {
...
async function handleDeleteThread(targetThreadId) {
  if (!confirm('确定要删除这条历史任务吗？')) return
