...
class StartWorkflowRequest(BaseModel):
    ...
    topic_direction: str = Field(
        ..., 
        description="宣传方向，例如：外部公众号、内部宣发平台、内部招聘平台、官网公示、技术成果宣传、活动复盘、内部公告",
        min_length=1,
        max_length=200
    )
...
class ResumeWorkflowRequest(BaseModel):
    ...
    action: Literal["select_topic", "approve", "reject"] = Field(
        ..., 
        description="操作类型：select_topic(选择宣传主题)、approve(通过审核)、reject(驳回)"
    )
...
async def start_workflow(
    ...
    """
    启动新的宣传任务（主题策划阶段：非流式 + 结构化输出 + token统计）
    
    接收宣传方向，启动工作流并运行到第一个中断点（主题策划阶段）。
    适用于外部公众号、内部宣发平台、内部招聘平台、官网公示等企业宣传内容流转场景。
    返回生成的5个宣传主题
        ...
        if request.action == "select_topic":
            ...
            if not request.data or "selected_topic" not in request.data:
                raise HTTPException(
                    ...
                    detail="选择宣传主题需要提供 selected_topic"
                )
            resume_value = {
...
# ============== SSE 流式输出请求模型 ==============
# 注意：流式输出仅用于宣传初稿生成场景（select_topic 和 reject 操作）
# 主题策划阶段和审核通过阶段不需要流式，请使用普通接口
...
class StreamResumeWorkflowRequest(BaseModel):
    ...
    仅支持需要宣传初稿生成的操作：
    - select_topic: 选择宣传主题后生成宣传初稿（需要流式）
    - reject: 驳回后重新生成宣传初稿（需要流式）
    
    审核通过(approve)请使用普通接口 POST /resume/{thread_id}
    """
    action: Literal["select_topic", "reject"] = Field(
        ..., 
        description="操作类型：select_topic(选择宣传主题后生成宣传初稿)、reject(驳回后重新生成宣传初稿)"
    )
    data: Optional[Dict[str, Any]] = Field(
        default=None,
...
async def stream_graph_updates(
    ...
    """
    使用 LangGraph astream_events 流式输出宣传初稿生成过程
    
    专门用于宣传初稿生成场景，提供 token 级别的流式输出
...
async def stream_resume_workflow(
    ...
    使用 Server-Sent Events (SSE) 实时返回 LLM 生成的宣传初稿（token 级别流式输出）
    
    支持的操作：
    - select_topic: 选择宣传主题后，流式生成宣传初稿
    - reject: 驳回审核后，流式重新生成宣传初稿
    ...
    async def generate():
        try:
            ...
            if request.action == "select_topic":
                if not request.data or "selected_topic" not in request.data:
                    yield format_sse_event("error", {"message": "选择宣传主题需要提供 selected_topic"})
                    return
                resume_value = {
                    "selected_topic": request.data["selected_topic"],
                }
