"""
主题策划子图模块 (Topic Selection Subgraph)

将宣传主题相关的逻辑封装为独立的子图，包含：
1. AI 生成宣传主题 (plan_topics)
2. 人工选择宣传主题 (human_select_topic) - 使用 interrupt
...
async def human_select_topic_node(state: AgentState) -> Command[Literal["__end__"]]:
    """
    人工选择宣传主题节点 (使用 LangGraph 1.0+ interrupt 模式)
    
    使用 interrupt() 暂停执行，等待人工输入宣传主题
...
def build_topic_selection_subgraph() -> StateGraph:
    ...
    子图工作流逻辑：
    1. Start -> plan_topics (AI 生成宣传主题)
    2. plan_topics -> human_select_topic (等待人工选择宣传主题，使用 interrupt())
    3. human_select_topic -> End (主题选择完成，返回主图)
